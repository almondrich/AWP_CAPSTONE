...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($action === 'approve') {
        // Update status to Approved and forward to College Dean
        $updateQuery = "UPDATE requisitions SET status = 'Approved', remarks = NULL WHERE requisition_id = ?";
        ...
        $updateQuery = "UPDATE requisitions SET status = 'Rejected', remarks = ? WHERE requisition_id = ?";
        $stmt = $con->prepare($updateQuery);
